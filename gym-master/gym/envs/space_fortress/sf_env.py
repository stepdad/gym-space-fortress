...
class SFEnv(gym.Env):
	...
	metadata = {'render.modes': ['rgb_array', 'human', 'human_sleep', 'minimal', 'minimal_sleep', 'minimal_debug', 'human_debug']}
	...
	def __init__(self, game='SFS'):
		...
		actions_SFS = {0: 65361, 1 : 65362, 2 : 65363, 3 : 32, 4 : 0}
		actions_AIM = {0 : 32,  1 : 65363, 2 : 65361, 3 : 0}
		actions_Control = {0: 65361, 1 : 65362, 2 : 65363, 3 : 0}
		...
		try:
			...
			print("Warning: Some functions were not found in the library.")
		try:
		...
		self.observation_space = gym.spaces.Box(0, 256, [84, 84, 1])
	...
	def _reset(self):
		self.reset_sf()
		screen = self.screen().contents
		img = np.ctypeslib.as_array(screen)
	...
	def save_image(self, path):
		new_frame = self.screen().contents
		img = np.ctypeslib.as_array(new_frame)
		img = np.reshape(img, (int(self.screen_height/self.scale), int(self.screen_width/self.scale), 1))
		filename = path+"/"+str(self.step_count)+".png"
